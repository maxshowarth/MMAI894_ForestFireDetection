...
from Model_Code import model_evaluation_utils as meu
from Model_Code import model_plot_utils as mpu
...
# Get available training sets from cloud and download ## NOTE: All training sets are being downloaded and used. If
# you only want to train on the fully augmented set uncomment the line below
...
def vgg_model_train():
    ...
    for training_set in training_sets:
        ...
        history = model.fit(train_x, train_y, batch_size=32, epochs=20, verbose=1, validation_split=0.2, shuffle=True,
                            callbacks=[early_stopping_monitor])

        mpu.plot_accuracy_loss(history, "./model_cache/train_data/{}_base_vgg16_plots.png".format(str(training_set)))
        upload_blob(bucket_name, "./model_cache/train_data/{}_base_vgg16_plots.png".format(str(training_set)),
                    "model_charts/{}_base_vgg16_plots.png".format(str(training_set)))

        model.save("./model_cache/train_data/{}_base_vgg16.h5".format(str(training_set)))
        upload_blob(bucket_name, "./model_cache/train_data/{}_base_vgg16.h5".format(str(training_set)),
                    "saved_models/{}_base_vgg16.h5".format(str(training_set)))
    ...
    for training_set in training_sets:
        ...
        early_stopping_monitor = EarlyStopping(patience=2)
        history = model_t.fit(train_x, train_y, batch_size=32, epochs=20, verbose=1, validation_split=0.2, shuffle=True,
                              callbacks=[early_stopping_monitor])
